...
export function Crossfader({
  ...
  // Calculate volume levels for each deck based on crossfader position
  // -1 = full A (A=100%, B=0%), 0 = center (A=50%, B=50%), 1 = full B (A=0%, B=100%)
  const deckAVolume = Math.max(0, Math.min(100, (1 - (position + 1) / 2) * 100));
  const deckBVolume = Math.max(0, Math.min(100, ((position + 1) / 2) * 100));
